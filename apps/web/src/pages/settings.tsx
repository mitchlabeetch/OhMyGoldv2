...
export default function SettingsPage() {
  ...
  const { profile, refreshProfile } = useAuthStore() as {
...
function ProfileTab({ profile, onSave }: { profile: { first_name?: string; last_name?: string; email?: string } | null; onSave?: () => void }) {
  ...
  async function handleSave() {
    ...
    if (user) {
      await supabase.from("user_profiles").update({ first_name, last_name }).eq("id", user.id);
    }
    setSaving(false);
    setSaved(true);
    onSave?.();
    ...
    <section className="bg-surface-card rounded-xl border border-border p-6 space-y-5">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <button className="text-sm text-gold-400 hover:text-gold-300 transition-colors">Upload photo</button>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="text-sm text-text-secondary font-medium" htmlFor="s-first-name">First name</label>
          <input
            id="s-first-name"
        ...
        <div className="space-y-1.5">
          <label className="text-sm text-text-secondary font-medium" htmlFor="s-last-name">Last name</label>
          <input
            id="s-last-name"
            className="w-full bg-surface-elevated border border-border rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-gold-500/50 outline-none"
            value={last_name}
...
function PasswordTab() {
  ...
  async function handleChange() {
    setError(null);
    if (!form.next || form.next.length < 8) return setError("New password must be at least 8 characters.");
  ...
  return (
    <section className="bg-surface-card rounded-xl border border-border p-6 space-y-4">
      ...
      {error && <p className="text-sm text-status-error bg-status-error/10 rounded-lg px-3 py-2">{error}</p>}
      {success && <p className="text-sm text-status-success bg-status-success/10 rounded-lg px-3 py-2">Password updated successfully.</p>}
      {(["current", "next", "confirm"] as const).map((field) => (
        <div key={field} className="space-y-1.5">
          <label className="text-sm text-text-secondary font-medium capitalize" htmlFor={`pw-${field}`}>
    ...
    <section className="bg-surface-card rounded-xl border border-border p-6 space-y-4">
      ...
      {Object.entries(prefs).map(([key, val]) => (
        <label key={key} className="flex items-center justify-between cursor-pointer py-1">
          ...
          >
            <span className={`block w-4 h-4 rounded-full bg-white mx-1 transition-transform ${val ? "translate-x-5" : "translate-x-0"}`} />
          </button>
        </label>
      ))}
    </section>
...
function LanguageTab() {
  ...
  return (
    <section className="bg-surface-card rounded-xl border border-border p-6 space-y-4">
      ...
      <div className="space-y-1.5">
        <label className="text-sm text-text-secondary font-medium" htmlFor="lang-select">Display language</label>
        <select
          id="lang-select"
          className="w-full bg-surface-elevated border border-border rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-gold-500/50 outline-none"
          value={lang}
    ...
    <section className="bg-surface-card rounded-xl border border-border p-6 space-y-4">
      ...
      <div className="flex items-center justify-between py-2 border-t border-border">
        <div>
          ...
          <div className="text-xs text-text-muted">Manage devices where you are logged in</div>
        </div>
