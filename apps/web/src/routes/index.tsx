...
function PageLoader() {
  return (
    ...
    >
      <Loader2 className="text-gold-400 h-8 w-8 animate-spin" />
    </div>
  );
}
  ...
  return (
    <Suspense fallback={<PageLoader />}>
      <Routes>
        ...
        {/* Admin routes */}
        {[
          { path: "/admin/dashboard", Component: AdminDashboard, roles: ["admin", "super_admin"] },
          { path: "/admin/locations", Component: AdminLocations, roles: ["admin", "super_admin"] },
          {
          ...
          { path: "/admin/users", Component: AdminUsers, roles: ["admin", "super_admin"] },
          { path: "/admin/users/:id", Component: AdminUserDetail, roles: ["admin", "super_admin"] },
          { path: "/admin/settings", Component: AdminSettings, roles: ["admin", "super_admin"] },
          { path: "/admin/audit-log", Component: AdminAuditLog, roles: ["admin", "super_admin"] },
          { path: "/admin/analytics", Component: AdminAnalytics, roles: ["admin", "super_admin"] },
        ].map(({ path, Component, roles }) => (
          <Route
            key={path}
            path={path}
            element={
              <ProtectedRoute roles={roles as AppRole[]}>
                <AppShell>
                  <Component />
                </AppShell>
              </ProtectedRoute>
        ...
        {/* Manager routes */}
        {[
          { path: "/manager/dashboard", Component: ManagerDashboard, roles: ["admin"] },
          { path: "/manager/members", Component: ManagerMembers, roles: ["admin"] },
          { path: "/manager/members/enroll", Component: ManagerMemberEnroll, roles: ["admin"] },
          { path: "/manager/members/:id", Component: ManagerMemberDetail, roles: ["admin"] },
          { path: "/manager/classes", Component: ManagerClasses, roles: ["admin"] },
          { path: "/manager/billing", Component: ManagerBilling, roles: ["admin"] },
        ].map(({ path, Component, roles }) => (
          <Route
            key={path}
            path={path}
        ...
        {[
          ...
          <Route
            ...
            path={path}
            element={
              <ProtectedRoute roles={["admin", "receptionist"] as AppRole[]}>
                <AppShell>
                  <Component />
                </AppShell>
              </ProtectedRoute>
        ...
        {[
          ...
          <Route
            ...
            path={path}
            element={
              <ProtectedRoute roles={["admin", "coach"] as AppRole[]}>
                <AppShell>
                  <Component />
                </AppShell>
              </ProtectedRoute>
        ...
        {[
          ...
          <Route
            ...
            path={path}
            element={
              <ProtectedRoute roles={["admin", "receptionist", "coach", "member"] as AppRole[]}>
                <AppShell>
                  <Component />
                </AppShell>
              </ProtectedRoute>
